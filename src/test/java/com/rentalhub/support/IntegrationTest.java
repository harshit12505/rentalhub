 ...
 *
 * After each test the tables and the caches are emptied, so no test can see another's
 * data. That matters for tests that are not {@code @Transactional}: they really
 * commit, because the behaviour under test (after-commit cache eviction, what a real
 * request sees) only happens when something commits.
 */
@SpringBootTest
